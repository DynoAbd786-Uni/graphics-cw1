...
{
	// Need to check if we are looking in correct memory space
	auto index = surface.get_linear_index(static_cast<uint32_t>(point.x), static_cast<uint32_t>(point.y));
	if (index < 0 || index > (surface.get_linear_index(surface.get_width() - 1, surface.get_height() - 1))) return 0;

	// Get point location in memory and check color
	auto const ptr = surface.get_surface_ptr() + index;
	return (ptr[0] > 0 || ptr[1] > 0 || ptr[2] > 0) ? 1 : 0;
}
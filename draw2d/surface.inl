...
void Surface::set_pixel_srgb( Index aX, Index aY, ColorU8_sRGB const& aColor )
{
	...
	int linearIndex = this->get_linear_index(aX, aY);

	// Each pixel in the mSurface array is represented by 4 bytes (R, G, B, and Alpha/unused)
	// Set the red, green, and blue channels from the given ColorU8_sRGB
	mSurface[linearIndex + 0] = aColor.r; // Red channel
	mSurface[linearIndex + 1] = aColor.g; // Green channel
	mSurface[linearIndex + 2] = aColor.b; // Blue channel
}
